...
⑦ข ระวัง "เขียวเพราะไม่มีของให้จับ":
   ...
   🔴 v1.2 (3 ส.ค. 69) — **กฎ ก เข้าร่วมวงนี้แล้ว**
      ตั้งแต่แก้หมุดเปล่า 3 จุดสุดท้าย กฎ ก จับได้ 0 จุด และรายการอนุญาตว่าง
      ⇒ ทั้งสี่กฎของด่านนี้ตอนนี้ "จับ 0" หมด ⇒ ⑦ข กลืนด่านนี้ทั้งด่าน
      ⇒ ของล่อประจำกฎคือ **สิ่งเดียว** ที่ทำให้รายงานที่เป็นศูนย์ล้วนยังอ่านได้
      ⛔ ห้ามลดของล่อลงด้วยเหตุผลว่า "ไม่มีของให้จับแล้ว" — นั่นคือตอนที่ต้องมีมากที่สุด
...
#    เหตุผล: หมุดเปล่า 3 จุดสุดท้าย (แก้ไปแล้วใน v1.2) อยู่ใน explanation ทั้งหมด
...
#
# ✅ **ว่างแล้ว ตั้งแต่ 3 ส.ค. 2569 (v1.2)** — รายการนี้ถึงปลายทางที่ตั้งใจไว้
#    3 จุดที่เคยอยู่ในรายการ ถูกแก้เป็น `[IMAGE:0]` ในคอมมิตเดียวกับที่ลบรายการนี้:
#       · samn-2555-01-q12         (วิชาสามัญ 2555 · ข้อสอบจริง)
#       · samn-2558-01-q27         (วิชาสามัญ 2558 · ข้อสอบจริง)
#       · chap-07-trigonometry-q22 (ชุดแต่งเอง)
#    ครูอนุมัติการแตะชุดข้อสอบจริง 2 ชุดนี้เมื่อ 3 ส.ค. 69 ("อนุมัติหมุดรูป 3 จุด")
#    ⇒ ตอนนี้ทั้งคลังมีหมุดเปล่า **0 จุด** ⇒ ตัวอ่านทั้งสองแบบ
#      (`/\\[IMAGE(?::(\\d+))?\\]/g` ของเรา และ `/\\[IMAGE:(\\d+)\\]/` แบบเข้ม)
#      ให้ชุดดัชนีเดียวกันเป๊ะทุกข้อ — ความกำกวมที่ด่านนี้ตั้งขึ้นมาเฝ้า **หมดไปแล้ว**
#
# 🔴 แล้วทำไมยังต้องมีรายการนี้อยู่ ทั้งที่มันว่าง
#    เพราะ "ว่าง" คือสถานะที่ต้อง **ถูกบังคับให้อยู่ต่อ** ⛔ ไม่ใช่ผลที่จบแล้ว
#    ⇒ หมุดเปล่าตัวต่อไปที่โผล่ขึ้นมา = แดงทันที (ไม่มีใบอนุญาตให้แล้ว)
#    ⇒ ถ้าจะเติมกลับ ต้องเติมพร้อมเหตุผล และ **ด่าน 10 จะเห็น** (ดูย่อหน้าถัดไป)
#
# ⚠️ v1.2 แก้คำอ้างที่ไม่จริงของ v1.1 ด้วย (กับดัก ⑦ก · ⑥)
#    v1.1 เขียนไว้ตรงนี้ว่า "รายการนี้ถูก ด่าน 10 เฝ้าอยู่" — **ไม่จริง**
#    ผมไปเปิด `scripts/check_config_growth.py` แล้วพบว่า `WATCHED` มี 4 รายการ
#    (check_bot_add.ALLOWED_PATHS · check_flawed_source.ALLOWED_KINDS ·
#     scan_symbols.WORD_BANNED · check_exit_codes.NOT_IN_CI) และ ⛔ ไม่มี ALLOW_BARE
#    ⇒ ประโยคนั้นเป็น **กติกาที่อยู่แต่ในคอมเมนต์** — คือของที่ไม่มีอยู่จริง
#      และมันอันตรายที่สุดพอดีตอนนี้ ที่รายการว่าง: รายการว่างที่โตกลับได้เงียบ ๆ
#      คือใบอนุญาตเปล่าที่รอให้ใครก็ได้กรอก
#    ⇒ v1.2 ลงทะเบียน ('scripts/check_image_markers.py', 'ALLOW_BARE') เข้า WATCHED จริง
#      ⇒ ตั้งแต่นี้ ประโยคข้างบนเป็นเรื่องจริงที่ตรวจสอบได้ ⛔ ไม่ใช่ความตั้งใจ
#    และด่านนี้เองบังคับ **ขาลง** ด้วย: รายการที่ค้าง = แดง (กับดัก ⑩)
#
# รูปแบบ: id -> (ฟิลด์ชั้นบนสุดที่หมุดอยู่, จำนวนจุดในข้อนั้น, เหตุผล)
# ⛔ ห้ามเติมรายการที่นี่โดยไม่มีใบอนุมัติของครู ถ้าข้อนั้นอยู่ในชุดข้อสอบจริง
# ─────────────────────────────────────────────────────────────
ALLOW_BARE = {}
MIN_REASON = 40
...
def selftest():
    ...
    #
    # ⚠️ v1.2 เปลี่ยนคำยืนยันของเคสนี้ เพราะ ALLOW_BARE ว่างแล้ว
    #    v1.1 เขียนว่า "ข้อมูลสะอาด + รายการอนุญาตค้าง 3 ข้อ ⇒ 1" — จริงตอนนั้น
    #    เพราะรายการมี 3 บรรทัดที่ไม่มีหมุดเปล่ารองรับในคลังจำลอง (ขาลงของ ⑩ ยิง)
    #    พอรายการว่าง เส้นทางเดียวกันให้ 0 ⇒ ถ้าไม่แก้ เคสนี้จะตกทันที
    #    ⇒ v1.2 แยกเป็นสองครึ่ง เพื่อ **ไม่ให้เสียกำลังตรวจ** ตอนรายการว่างลง:
    #       ⑭ก ข้อมูลสะอาด + รายการว่าง ⇒ 0 (ทางที่คลังจริงเดินอยู่ตอนนี้)
    #       ⑭ข ข้อมูลสะอาดชุดเดิม แต่ฉีดรายการอนุญาตค้างเข้าไป ⇒ ยังต้อง 1
    #          ⇒ ขาลงของกฎ ก ยังถูกทดสอบที่ระดับ run() ⛔ ไม่ใช่แค่ในของล่อ
    with tempfile.TemporaryDirectory() as t:
        _write_repo(t, {'a.json': {'questions': [
            _q('z-q1', explanation=['[IMAGE:0]'], imageSpec={'t': 1})]}})
        code, out = run(t)
        txt = '\n'.join(out)
        eq('⑭ก ข้อมูลสะอาด + รายการอนุญาตว่าง ⇒ 0', code, 0)
        eq('⑭ก พิมพ์ตัวหาร', '⑨ ตัวหาร' in txt and 'ข้อ 1' in txt, True)
        eq('⑭ก พิมพ์ทั้งสี่กฎ',
           all(k in txt for k in ['กฎ ก', 'กฎ ข', 'กฎ ค', 'กฎ ง']), True)
        # เลขรายการอนุญาตที่พิมพ์ ต้องเป็นเลขของรายการจริง ⛔ ไม่ใช่ค่าคงที่ในข้อความ
        eq('⑭ก พิมพ์จำนวนรายการอนุญาตตามของจริง',
           ('รายการอนุญาต %d ข้อ' % len(ALLOW_BARE)) in txt, True)

        # ⛔ ห้ามฉีดผ่าน verdict= — ตัวเดียวกันนี้ถูกใช้ยิงของล่อด้วย
        #    (run() เรียก run_decoys(decoys, _scan, _verdict)) ⇒ ของล่อลบของกฎ ก
        #    จะโดนรายการค้างเล่นงาน ⇒ "กฎยิงมั่ว" ⇒ ได้ 2 แทนที่จะเป็น 1
        #    ⇒ สลับตัวแปรระดับโมดูลชั่วคราวแทน แล้วคืนค่าใน finally
        _real_allow = globals()['ALLOW_BARE']
        globals()['ALLOW_BARE'] = dict(_DECOY_ALLOW_STALE)
        try:
            code2, out2 = run(t)
        finally:
            globals()['ALLOW_BARE'] = _real_allow
        eq('⑭ข ข้อมูลสะอาด + ฉีดรายการอนุญาตค้าง ⇒ 1', code2, 1)
        eq('⑭ข รายงานสั่งให้ลบบรรทัดที่ค้าง',
           any('รายการอนุญาตค้าง' in x for x in out2), True)
        eq('⑭ข คืนค่ารายการจริงแล้ว', ALLOW_BARE is _real_allow, True)
