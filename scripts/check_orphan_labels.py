...
㉑ ตัวเลขที่ต้องประกาศ (ด่านที่เขียนจากเคสเดียว มักไม่จับเคสนั้น)
  ...
  · **ผลกับเคสต้นเรื่อง** : `pat1-2560-03-q35` **ติดธงจริงตั้งแต่วันแรก**
                 ⇒ ด่านนี้จับเคสที่ทำให้มันเกิดได้จริง ⛔ ไม่ใช่ด่านที่เขียนแล้วเงียบ

  · **อัปเดต 4 ส.ค. 2569** : ครูเซ็นอนุมัติให้แก้ป้ายของเคสต้นเรื่องแล้ว
                 (22.5 → 62 · 2 จุด) ⇒ ธงของข้อนั้นหายไปจากคลังจริง
                 และบรรทัดของมันถูกลบออกจาก PINNED **ในคอมมิตเดียวกัน**
                 ⇒ ตัวเลขปัจจุบัน: **จับ 1 · ปักไว้ 1 · แดง 0**
                 (เหลือเฉพาะ `samn-2565-03-q30` ที่อ่านแล้วว่าไม่ใช่ของเสีย)
...
# ─────────────────────────────────────────────────────────────
PINNED = {
    # ⛔ เคยมีบรรทัด 'pat1-2560-03-q35' อยู่ตรงนี้ — ถูกลบออกในคอมมิตเดียวกับที่
    #    แก้ป้าย 22.5 → 62 ในไฟล์ข้อมูล (ครูเซ็นอนุมัติ 4 ส.ค. 2569 · รายการที่ ③)
    #    ⇒ นี่คือขาลงของกฎ ⑩ ที่ทำงานตามที่ออกแบบ: ของหายจากคลัง ⇒ บรรทัดต้องหายด้วย
    #      ถ้าลืมลบ ด่านนี้จะแดงเองด้วยข้อความ "รายการปักค้าง"
    'samn-2565-03-q30': (
        ('16', '28', '40'),
...
def selftest():
    ...
    #
    # ⚠️ คลังจำลองในเคสนี้ **ไม่มี** ข้อที่ปักไว้ของคลังจริงสักข้อ
    #    ⇒ ถ้าปล่อยให้ PINNED จริงมีผล ขาลงของกฎ ข จะยิง (รายการปักค้างทุกบรรทัด)
    #      แล้วเคสนี้จะได้ 1 ทั้งที่กำลังทดสอบเรื่อง "พิมพ์ตัวหาร" คนละเรื่องกัน
    #    ⇒ สลับรายการปักเป็นว่างชั่วคราว แล้วคืนค่าใน finally
    #    ⛔ ห้ามแก้ด้วยการเติมข้อที่ปักไว้แบบปลอมลงคลังจำลอง —
    #      นั่นคือการผูก self-test กับเนื้อหาของคลังจริง (กับดัก ⑰)
    #
    # 🔴 ผลพลอยได้ที่ตั้งใจ: ตอนนี้ค่าที่ "พิมพ์" (0) ต่างจากค่าจริงในไฟล์ (ไม่ใช่ 0)
    #    ⇒ ถ้าใครฮาร์ดโค้ดเลขรายการที่ปักไว้ลงในข้อความรายงาน เคสนี้จะจับได้
    #    ⛔ ห้ามเขียนเลขจำนวนบรรทัดของ PINNED ตายตัวลงในเคสนี้ — วันที่ครูเซ็นใบแก้
    #      แล้วบรรทัดหนึ่งถูกลบ (เกิดจริง 4 ส.ค. 69) เคสจะล้มทั้งที่ด่านยังถูกต้อง
    #      ⇒ ตรวจแค่ "ไม่ว่าง" ซึ่งเป็นเงื่อนไขเดียวที่ข้อสรุปข้างบนต้องใช้
    with tempfile.TemporaryDirectory() as t:
        ...
        eq('⑬ รายการจริงไม่ว่าง ⇒ 0 ข้างบนมาจากตัวแปรสด ไม่ใช่ค่าคงที่',
           len(PINNED) > 0, True)
