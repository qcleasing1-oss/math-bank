# 📋 HANDOVER — Project คลังข้อสอบ (BANK) v3

> **เขียนเมื่อ:** 16 พฤษภาคม 2569 (2026-05-16)
> **เขียนโดย:** Claude (chat restart — แชทก่อนเต็ม context)
> **สำหรับ:** Claude คนถัดไปที่จะ work ใน project นี้
> **คุณครู:** คุณครูOFF (ครูคณิต ม.ปลาย)
> **Supersedes:** `HANDOVER_BANK_v2.md`

---

## 🔄 เหตุผลที่มี v3

แชทก่อนเริ่มไปแล้ว แต่ยังไม่ได้แก้อะไร (เพิ่ง health check ก็เจอ "conversation too long" เพราะ project knowledge หนัก) — **สถานะ codebase = เหมือนตอนจบ Phase B ทุกอย่าง** v3 นี้แค่ refresh metadata + เพิ่ม note ป้องกันแชทเต็มซ้ำ

### 💡 Tip ป้องกันแชทเต็มเร็ว (สำหรับ Claude คนใหม่)
- **อย่าโหลด `bank.json` เต็มไฟล์เข้า context** ถ้าไม่จำเป็น — ใช้ `project_knowledge_search` แทน หรือขอคุณครูส่งเฉพาะข้อที่จะแก้
- **อย่า view `index__1_.html` (portal เก่า) ทั้งไฟล์** — มันใหญ่มาก ดูเฉพาะ line range ของ renderer ที่ต้องการ port
- 1 atomic task / turn = ทำงานได้ยาวกว่า
...
│   └── renderers.js    ← Phase B — SVG renderers, port จาก portal
├── docs\
│   ├── HANDOVER_BANK_v1.md  ← เก่า
│   ├── HANDOVER_BANK_v2.md  ← เก่า (15 พ.ค.)
│   └── HANDOVER_BANK_v3.md  ← ไฟล์นี้ (16 พ.ค.)
├── .gitattributes
└── README.md
```
...
⚠️ **อย่า view portal ทั้งไฟล์** (ใหญ่มาก ทำให้แชทเต็มเร็ว) — ใช้ `view_range` เปิดเฉพาะ line ของ renderer ที่จะ port
...
11. **🆕 Context budget** — ระวังแชทเต็ม! อย่าโหลด bank.json/portal ทั้งไฟล์ ใช้ search หรือ view_range
...
1. `HANDOVER_BANK_v3.md` (ไฟล์นี้) — อ่านก่อน
2. `bank.json` (จาก `data/bank.json`) — เมื่อจำเป็น

**Optional (เฉพาะ Phase E — เพิ่ม renderer ใหม่):**
3. Portal `index__1_.html` — สำหรับ port renderer functions (อ่านเฉพาะ line range)
4. `viewer/renderers.js` ปัจจุบัน — ดู pattern ของ renderer ที่มีอยู่

**ไม่ต้องแนบ:**
- `HANDOVER_BANK_v1.md`, `HANDOVER_BANK_v2.md` (เก่า — content ครอบคลุมใน v3 แล้ว)
...
ต่อยอด project "คลังข้อสอบคณิต ม.ปลาย" จากแชทก่อน (chat เต็ม context restart)

แนบไฟล์:
- HANDOVER_BANK_v3.md — อ่านก่อน (สถานะปัจจุบัน + bank schema + URLs + workflow)
- bank.json — bank ปัจจุบัน

วันนี้อยากทำ Phase _____ [ D / E — เลือก ]

ทำตาม anti-crash protocol — 1 atomic task per turn
อย่าโหลด bank.json/portal เต็มไฟล์เข้า context (ป้องกันแชทเต็มเร็ว)
```
...
- Phase A: "ไม่เคยใช้ GitHub" → "deploy public website ผ่าน Pages เป็นเอง" ในเวลาไม่กี่ชั่วโมง
- Phase B: debug `file://` → port shadow → cache → ได้ครบเอง โดยมี Claude guide
...
— Claude (16 พ.ค. 2569, chat restart v3)
