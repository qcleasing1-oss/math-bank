# -*- coding: utf-8 -*-
"""ด่านตรวจไฟล์ข้อเดี่ยวของก้อน 21 — ผู้แต่งแต่ละคนรันกับไฟล์ของตัวเองก่อนส่งงาน

    python3 /home/claude/k2/check_part_b21.py 105
...
from stems_b21 import STEMS

SPEC = {s[0]: s for s in STEMS}
n = int(sys.argv[1])
p = '/home/claude/k2/parts_b21/q%03d.py' % n
bad = []
...
for kw in ['Rubric', 'Scoring was written before the level was read off.',
           'sympy (verify_b21.py)', 'Collision sweep']:
    if kw not in nt:
        fail('notes ขาดส่วน %r' % kw)
# ⛔ notes เป็นภาษาอังกฤษล้วน — ยกเว้นชื่อระดับสี่คำ ซึ่งแม่แบบก้อน 19 เขียนเป็นคำไทย
#    (`-> level: ง่าย.`) ⇒ ต้องคงธรรมเนียมเดิมไว้ ไม่ให้ก้อน 21 ลอยจากก้อนก่อน
nt_probe = nt
...
if not re.search(r'-> level: (ง่าย|ปานกลาง|ยากมาก|ยาก)', nt):
    fail('notes ต้องมี "-> level: <ชื่อระดับภาษาไทย>" ตามธรรมเนียมก้อน 19')
